"""Recursive list boxing/unboxing for PortaPy's native Python facade."""
...
def install() -> None:
    ...
    def bind(library: _native._NativeLibrary) -> None:
        ...
        library._function(
            ...
            [_native._U64, _native._U64, _native._SIZE, _native._U64],
        )
        library._function(
            "portapy_list_append",
            [_native._U64, _native._U64, _native._U64],
        )
    ...
    def box(environment: _native.NativeEnvironment, value: object) -> int:
        ...
        try:
            ...
            result = environment._new_value(
                environment._api.portapy_value_from_list,
                pointer,
                len(handles),
            )
            _ = storage
    ...
    def unbox(environment: _native.NativeEnvironment, handle: int) -> object:
        ...
        if kind.value != _LIST_KIND:
            return original_unbox(environment, handle)

        size = _native._SIZE(0)
        environment._check(
            int(
                environment._api.portapy_list_get_size(
                    environment._runtime,
                    _native._U64(handle),
                    ctypes.byref(size),
                )
        ...
        for index in range(size.value):
            ...
            environment._check(
                int(
                    environment._api.portapy_list_get_item(
                        environment._runtime,
                        _native._U64(handle),
                        index,
                        ctypes.byref(item),
            ...
            try:
                ...
                if status != int(Status.OK):
                    environment._check(status, "release list item")
        return values
