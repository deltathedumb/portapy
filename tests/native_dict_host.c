...
int main(int argc, char **argv) {
    ...
    const char source[] =
        ...
        "duplicate = {\"a\": 1, \"a\": 42}[\"a\"]\n"
        "nested = {\"outer\": {\"inner\": 42}}[\"outer\"][\"inner\"]\n";
    if (!execute(exec_utf8, runtime, source)) return 13;
    if (!evaluate_i64(eval_utf8, as_i64, release, runtime, "answer", 40)) return 14;
    if (!evaluate_i64(eval_utf8, as_i64, release, runtime, "empty", 0)) return 15;
    if (!evaluate_i64(eval_utf8, as_i64, release, runtime, "duplicate", 42)) return 16;
    if (!evaluate_i64(eval_utf8, as_i64, release, runtime, "nested", 42)) return 17;
    if (!evaluate_i64(eval_utf8, as_i64, release, runtime, "len(values)", 2)) return 18;
    if (!evaluate_bool(eval_utf8, as_bool, release, runtime, "not {}", 1)) return 19;
    if (!evaluate_bool(eval_utf8, as_bool, release, runtime, "{\"a\": 1, \"b\": 2} == {\"b\": 2, \"a\": 1}", 1)) return 20;
    if (!evaluate_status(eval_utf8, runtime, "{\"a\": 1}[\"missing\"]", PORTAPY_RUNTIME_ERROR)) return 21;
