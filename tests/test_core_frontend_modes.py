...
def test_portable_frontend_eval_mode_returns_expression_value() -> None:
    namespace: dict[str, object] = {}
    result = VirtualMachine().run(
        compile_portable_source("sum(range(7)) * 2", mode="eval"),
        namespace,
    )
    assert result == 42
    assert "__portapy_compiled_result" not in namespace
...
def test_portable_frontend_single_expression_is_interactive() -> None:
    code = compile_portable_source("40 + 2", mode="single")
    assert code.interactive is True
    assert VirtualMachine().run(code, {}) == 42
...
def test_portable_frontend_single_statement_executes() -> None:
    namespace: dict[str, object] = {}
    code = compile_portable_source("answer = 40 + 2\n", mode="single")
    assert code.interactive is True
    VirtualMachine().run(code, namespace)
    assert namespace["answer"] == 42


def test_portable_frontend_rejects_unknown_mode() -> None:
    with pytest.raises(ValueError, match="unsupported compile mode"):
        compile_portable_source("42", mode="invalid")
