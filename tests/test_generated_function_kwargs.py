...
def test_generated_source_contains_keyword_variadic_dictionary_packing(tmp_path: Path) -> None:
    _, names, paths = _api(tmp_path)
    try:
        source = paths[3].read_text(encoding="utf-8")
        assert "_PARAMETER_VAR_KEYWORD = 4" in source
        assert "def _var_keyword_index(" in source
        assert "def _build_kwargs_dict(" in source
        assert "_scalar_string_from_text(" in source
        assert "kwarg_positions: list[int] = []" in source
    finally:
        for name in reversed(names):
            sys.modules.pop(f"portapy.{name}", None)


def test_empty_and_nonempty_kwargs_are_real_dictionaries(tmp_path: Path) -> None:
...
def test_kwargs_mix_with_fixed_varargs_defaults_and_keyword_only(tmp_path: Path) -> None:
    api, names, _ = _api(tmp_path)
    try:
        runtime = api._portapy_runtime_create_impl()
        source = (
        ...
        assert _eval_int(api, runtime, "mixed") == 64
        assert _eval_int(api, runtime, "keyword_middle") == 48
    finally:
        for name in reversed(names):
            sys.modules.pop(f"portapy.{name}", None)


def test_kwargs_survive_nested_calls_and_local_restore(tmp_path: Path) -> None:
    api, names, _ = _api(tmp_path)
    try:
        runtime = api._portapy_runtime_create_impl()
        source = (
            ...
            "def outer(**rest):\n"
            '    return pick(first=rest["first"], last=rest["last"])\n'
            "answer = outer(first=20, last=22)\n"
        )
        assert _exec(api, runtime, source) == base.PORTAPY_OK
...
def test_kwargs_call_and_definition_errors_are_structured(tmp_path: Path) -> None:
    api, names, _ = _api(tmp_path)
    try:
        runtime = api._portapy_runtime_create_impl()
        assert _exec(api, runtime, "def collect(**items):\n    return len(items)\n") == base.PORTAPY_OK
        assert _exec(api, runtime, "def strict(value):\n    return value\n") == base.PORTAPY_OK

        duplicate = api._portapy_eval_span_impl(
            runtime,
            "collect(item=1, item=2)",
            len("collect(item=1, item=2)"),
        )
        assert duplicate == 0
        assert api._portapy_last_status_impl() == base.PORTAPY_TYPE_ERROR

        unexpected = api._portapy_eval_span_impl(
            runtime,
            "strict(value=1, extra=2)",
            len("strict(value=1, extra=2)"),
        )
        assert unexpected == 0
        assert api._portapy_last_status_impl() == base.PORTAPY_TYPE_ERROR
