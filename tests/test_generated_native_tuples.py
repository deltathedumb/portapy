...
def _api(tmp_path: Path):
    names = (
        "_tuple_scalar_test",
        "_tuple_expression_test",
        "_tuple_control_test",
        "_tuple_function_test",
    )
...
def test_generated_tuple_source_is_static_and_namespaced(tmp_path: Path) -> None:
    _, names, paths = _api(tmp_path)
    try:
        scalar = paths[0].read_text(encoding="utf-8")
        expression = paths[1].read_text(encoding="utf-8")
        assert "PORTAPY_VALUE_TUPLE = 8" in scalar
        assert "def _scalar_append_tuple(" in scalar
        assert "def _scalar_tuple_get(" in scalar
        assert "def _scalar_sequence_length(" in scalar
        assert "_scalar_tuple_size_unchecked," in expression
        assert "kind == 8" in expression
    finally:
        for name in reversed(names):
            sys.modules.pop(f"portapy.{name}", None)


def test_tuple_literals_grouping_indexing_and_length(tmp_path: Path) -> None:
    api, names, _ = _api(tmp_path)
    try:
        runtime = api._portapy_runtime_create_impl()
        assert _eval_int(api, runtime, "(1 + 2)") == 3
        ...
        assert _eval_int(api, runtime, "len((42,))") == 1
        assert _eval_int(api, runtime, 'len("é")') == 1
        assert _eval_int(api, runtime, 'len(b"abc")') == 3
    finally:
        for name in reversed(names):
            sys.modules.pop(f"portapy.{name}", None)


def test_tuple_truthiness_and_structural_equality(tmp_path: Path) -> None:
    api, names, _ = _api(tmp_path)
    try:
        runtime = api._portapy_runtime_create_impl()
...
def test_tuple_index_errors_are_structured(tmp_path: Path) -> None:
    api, names, _ = _api(tmp_path)
    try:
        runtime = api._portapy_runtime_create_impl()
        ...
        assert api._portapy_error_type_size_impl(runtime) == len("IndexError")
...
def test_tuples_work_in_native_functions_and_control_flow(tmp_path: Path) -> None:
    api, names, _ = _api(tmp_path)
    try:
        runtime = api._portapy_runtime_create_impl()
        source = (
