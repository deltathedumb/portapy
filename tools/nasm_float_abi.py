"""Append C-ABI wrappers for PortaPy's Python-authored float handles.

The Python implementation owns kind, lifetime, and payload semantics. These
wrappers only bit-cast IEEE-754 binary64 values between XMM registers and the
integer payload accepted by the compiled Python core.
"""
...
def append_float_abi(source: str, *, target: str) -> str:
    labels = _labels(source)
    missing = [name for name in _REQUIRED_IMPLS if name not in labels]
    if missing:
        raise ValueError(
            "generated native API is missing float implementation label(s): "
            + ", ".join(missing)
        )
    if target == "linux":
