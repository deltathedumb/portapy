"""Preserve function parameter names through native frontend lowering.
...
function argument binding. Replace all parameter-name comprehensions and
conditional attribute expressions with explicitly typed string loops.
"""
...
_ARGUMENTS_OLD = '''            function_arguments = list(node.args.posonlyargs)
            for argument in node.args.args:
                function_arguments.append(argument)
            nested = _Lowerer(node.name, [arg.arg for arg in function_arguments])
'''

_ARGUMENTS_NEW = '''            function_arguments = list(node.args.posonlyargs)
            for argument in node.args.args:
                function_arguments.append(argument)
...
def _state(source: str, old: str, new: str, label: str) -> str:
    ...
    if old_count == 0 and new_count == 1:
        return "normalized"
    raise RuntimeError(
        f"native function {label} parameter source shape changed: "
...
def main() -> int:
    ...
    argument_state = _state(source, _ARGUMENTS_OLD, _ARGUMENTS_NEW, "regular")
    special_state = _state(source, _SPECIAL_OLD, _SPECIAL_NEW, "special")
    argument_count = 0
    special_count = 0
    if argument_state == "original":
        source = source.replace(_ARGUMENTS_OLD, _ARGUMENTS_NEW, 1)
        argument_count = 1
    if special_state == "original":
        source = source.replace(_SPECIAL_OLD, _SPECIAL_NEW, 1)
        special_count = 1
    ...
    if _ARGUMENTS_OLD in source or _SPECIAL_OLD in source:
        raise RuntimeError("unsafe native function parameter extraction remains")
    print("NORMALIZED NATIVE FUNCTION PARAMETER NAMES", argument_count, special_count)
    return 0
