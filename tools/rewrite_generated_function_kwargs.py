"""Add native ``**kwargs`` binding backed by owned dictionaries."""
from __future__ import annotations

from pathlib import Path

from tools.rewrite_generated_function_varargs import (
    _VARARGS_BINDING_BLOCK,
    _VARARGS_DEFAULT_CONDITION,
    _VARARGS_PACK_INSERTION,
    _varargs_invoke,
)
...
_KWARGS_BINDING_BLOCK = r'''    positional = 0
    ...
    while index < argument_count:
        ...
        if name == "":
            ...
            if target < 0:
                capture_keyword = kwargs_index >= 0
                if not capture_keyword:
                    error_message = "unexpected keyword argument"
            else:
                kind = _parameter_kind(parameters, target)
                if (
                    kind == _PARAMETER_POSITIONAL_ONLY
                    or kind == _PARAMETER_VAR_POSITIONAL
                    or kind == _PARAMETER_VAR_KEYWORD
                ):
                    capture_keyword = kwargs_index >= 0
                    if not capture_keyword:
                        if kind == _PARAMETER_POSITIONAL_ONLY:
            ...
            if capture_keyword:
                duplicate = False
                duplicate_index = 0
                while duplicate_index < len(kwarg_positions):
                    previous = argument_start + kwarg_positions[duplicate_index]
                    if _call_argument_names[previous] == name:
                        duplicate = True
                    duplicate_index += 1
                if duplicate:
                    error_message = "multiple values for keyword argument"
                else:
...
def _parameter_kind_and_kwargs_builder() -> str:
...
def _build_kwargs_dict(
    ...
    value = _append_value(runtime, 9, len(positions))
    if value == 0:
        return 0
    index = 0
    while index < len(positions):
        ...
        _scalar_dict_entry_key.append(_call_argument_names[argument_index])
    ...
    while position <= end:
        ...
        if split:
            ...
            if text == "/":
                ...
                check = 0
                while check < count:
                    if _parameter_at(previous, check) == source[name_bounds[0]:name_bounds[1]]:
                        return ["", name_bounds[0], PORTAPY_COMPILE_ERROR]
                    check += 1
                saw_kwargs = True
                count += 1
...
def _kwargs_invoke() -> str:
    ...
    if _VARARGS_PACK_INSERTION not in source:
        raise ValueError("varargs packing block has an unexpected implementation")
...
def rewrite_generated_function_kwargs(path: Path) -> Path:
    ...
    source = _replace_function(
        source,
        "_parameter_kind",
        _parameter_kind_and_kwargs_builder(),
    )
